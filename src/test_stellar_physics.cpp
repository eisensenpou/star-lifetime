...
void test_lifetime_estimates() {
  ...
  double rel_error =
      num_analysis::errors::relative_error(expected_life, numeric_life);
  ...
  std::cout
      << "PASS: Analytical lifetime model consistent with numeric result.\n";
}
...
void test_stellar_models() {
  ...
  std::cout << "PASS: Numerical stellar lifetime integration executed "
               "successfully.\n";
}
