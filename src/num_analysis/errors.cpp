...
double relative_error(double exact, double approx) {
  ...
  if (exact == 0)
    throw std::invalid_argument(
        "Exact value cannot be zero in relative error calculation.");
...
double simpson_error(double a, double b, int n, double max_fourth_derivative) {
  ...
  double h = (b - a);
  return (std::pow(h, 5) / (180.0 * std::pow(n, 4))) *
         std::abs(max_fourth_derivative);
}
...
// =====================================================
double trapezoid_error(double a, double b, int n,
                       double max_second_derivative) {
  /*************************
  ...
  if (n <= 0)
    throw std::invalid_argument("Number of subintervals n must be positive.");
  double h = (b - a);
  return (std::pow(h, 3) / (12.0 * std::pow(n, 2))) *
         std::abs(max_second_derivative);
}
