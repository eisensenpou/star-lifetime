...
double fuel_stock(double M) {
  /**********************
   * Estimate fuel stock, i.e. the hydrogen fuel available for fusion in the
   * star
...
double lifetime(double M) {
  /***********************
   * Estimate the lifetime of the star, i.e. the time it takes for the star to
   * burn all its fuel (Hydrogen)
   * @param M: Stellar mass in kg
...
double estimate_lifetime(double S0, double (*L)(double), double T_guess, int n,
   ...
   * @param L           luminosity function pointer (takes double t, returns
   * double)
   ...
   *************************/
  double burned =
      use_simpson ? num_analysis::integration::simpsons_rule(L, 0, T_guess, n)
...
Star::Star(const std::string &name, double mass_Msun, double luminosity_Lsun,
  ...
  if (luminosity_Lsun < 0.0) {
    throw std::invalid_argument(
        "Star::Star - luminosity_Lsun must be non-negative");
  }
  if (observed_age_years < 0.0) {
    throw std::invalid_argument(
        "Star::Star - observed_age_years must be non-negative");
  }
...
void Star::setMass(double mass_Msun) {
  ...
  if (mass_Msun < 0.0) {
    throw std::invalid_argument(
        "Star::setMass - mass_Msun must be non-negative");
  }
  this->mass_Msun = mass_Msun;
}
...
void Star::setLuminosity(double luminosity_Lsun) {
  ...
  if (luminosity_Lsun < 0.0) {
    throw std::invalid_argument(
        "Star::setLuminosity - luminosity_Lsun must be non-negative");
  }
  this->luminosity_Lsun = luminosity_Lsun;
}
...
void Star::setAge(double observed_age_years) {
  ...
  if (observed_age_years < 0.0) {
    throw std::invalid_argument(
        "Star::setAge - observed_age_years must be non-negative");
  }
  this->observed_age_years = observed_age_years;
}
...
void Star::printSummary() const {
  ...
  std::cout << "Luminosity: " << luminosity_Lsun << " L☉ ("
            << this->luminosityInWatts() << " W)\n";
  std::cout << "Observed Age: " << this->observed_age_years << " years\n";
  std::cout << "Fuel Lifetime Estimate: " << this->computeFuelLifetime()
            << " years\n";
  std::cout << "----------------------------------------\n";
} // end of printSummary
